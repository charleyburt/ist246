...
public:
  ...
  /**
   * Searches the tree for an element and returns true (false)
   * if the element is (is not) in the tree
   */ 
...
private:
   ...
   */
  bool contains(const T& k, Node *pCurr) const;
  
  
  /**
   * Recursive helper function for height
   */  
  unsigned height(Node *pCurr) const;
 ...
 */
template <class T> bool Tree<T>::contains(const T& k) const{
  
  return contains(k, this->pRoot);
} 
 ...
 * Recursive helper function for Contains
 */
template <class T> bool Tree<T>::contains(const T& k, Tree<T>::Node *pCurr) const{
  using namespace std;
  
// uncommenting the following makes it easier to see what is happening
/*  //print out what we are checking
  ...
  if (pCurr->data == k){
    ...
    while (i != pCurr->children.end()) {
      //check and see if it is what we are looking for
      if (contains(k,*i) == true){
        return true;
      }else{
        //check the next one
        i++;
 ...
 * Recursive helper function for height
 */
template <class T> unsigned Tree<T>::height(Tree<T>::Node *pCurr) const{
  using namespace std;
    ...
    while (i != pCurr->children.end()) {
      //set test equal to the current "highest" and call height again
      test = max(test, height(*i));
      //go to the next one
      i++;
    }
    
    //add one to the height for each node(aka each level)
    test ++;
