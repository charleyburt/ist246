...
public:
  ...
  friend std::ostream &operator<<(std::ostream &out, const SimpleBST<T> &tree) {
    out << "[";
    tree.inOrderPrint(out, tree.pRoot);
    out << "]";
    return out;
  }
  ...
  /**
   * Print the binary search tree as a horizontally-oriented ASCII
   * tree diagram.
   */
  ...
  /**
    * returns the true id the tree is proper
    * false if the true is not proper
  */
  bool isProper() const;
...
private:
   ...
   */
  void inOrderPrint(std::ostream &out, Node *pCurr) const;
  ...
  Node *remove(const T &val, Node *pCurr);
    ...
    /**
   * Recursive helper function for isProper
   */
  bool isProper(Node *pCurr) const;
};
 ...
 */
template <class T>
void SimpleBST<T>::inOrderPrint(std::ostream &out, Node *pCurr) const {

  if (pCurr != 0) {
    inOrderPrint(out, pCurr->pLeft);
    out << pCurr->data << " ";
    inOrderPrint(out, pCurr->pRight);
  }
}
 ...
 */
template <class T>
void SimpleBST<T>::Node::print(std::string prefix, bool isTail) {
  using namespace std;
 ...
 */
template <class T> void SimpleBST<T>::printAsTree() const {
  using namespace std;
...
/**
 * isProper function implementation
 */
template <class T> bool SimpleBST<T>::isProper() const{
  
  return isProper(pRoot);
} 
 ...
 * Recursive helper function for isProper
 */
template <class T> bool SimpleBST<T>::isProper(Node *pCurr) const{
  ...
  //else it does have children we need to check
  else {
    
    //if the left one has children
    if(pCurr -> pLeft != 0){
      
      //if the right one does not have children
      if(pCurr -> pRight == 0){
        return false;
      }
      
      //else they both have children
      else {
        
        //call the function again on both children, if either returns false
        if(isProper(pCurr->pLeft) == false || isProper(pCurr->pRight) == false){
          return false;
       }
       
       //else they both returned true
       else{
         return true;
       }
      }
    ...
    //else the left does not have children
    else {
      
      // if the right does not have children
      if(pCurr -> pRight == 0){
        return true;
      } 
      
      //else the right does have children
      else {
        return false;
      }
    }
