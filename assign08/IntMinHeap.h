...
//Charley and Tram
//Modified Class by adding a constructor to enforce the min-heap and a toSortedArray function
//11/16/2017
...
public:
  ...
  /**
   * Constructor for new IntMinHeap
   * Takes a pointer to an array of ints, and an unsigned value representing the size of the array. 
   * The constructor should copy the values from the array into the heap's vector, and then enforce the min-heap property. 
   */
   ...
   * Used to format the min-heap as array
   * Uses successive calls to removeMin() to fill an array of integers
   */
...
/*
 * toSortedArray function
 * pA pointer to array, n represents the size of the array 
 */
...
IntMinHeap::IntMinHeap( int* pA, unsigned n ) {
    ...
    // copy values from pA and save them into the Heap's vector
    for ( unsigned i = 0u; i < n; i++) {
    ...
    for ( unsigned i = vec.size()/2; i >= 1; i--) {
